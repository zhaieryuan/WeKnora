...
这将启动完整的本地开发基础设施：
- PostgreSQL（数据库）与 Redis（缓存）
- DocReader（文档读取服务）与 MinIO（对象存储）
- Qdrant、OpenSearch、Milvus（可切换的向量/检索后端）
- Neo4j（图数据库）、Dex（OIDC）与 SearxNG（联网搜索）
- Langfuse 及其 ClickHouse、MinIO、Worker 依赖

首次启动会拉取多个镜像，并占用更多本机 CPU、内存、磁盘和端口。启动命令会使用 Docker Compose 的就绪检查等待已选服务，默认上限为 180 秒；资源较少的机器可在 `.env` 中设置更大的正整数 `DEV_START_WAIT_SEC`。需要使用支持 `up --wait` 的当前版本 Docker Compose。

`sandbox`、`searxng-init` 与 `langfuse-db-init` 是一次性任务，成功完成后显示为 `Exited (0)` 属于正常现象。其余默认服务退出、健康检查失败或等待超时时，`make dev-start` 会返回失败并打印容器状态，不应继续启动本地后端。

如需缩小本机资源占用，可显式不启动 Langfuse；OpenDataLoader hybrid 与 OpenSearch Dashboards 仍是按需服务：
...
- **Milvus**: localhost:19530（健康检查: localhost:9091）
...
A: 确保先运行了 `make dev-start` 且命令成功返回。该命令会等待 Compose 服务就绪；如果超时，请先根据它打印的容器状态排错，或在机器资源不足时适当调大 `DEV_START_WAIT_SEC`。
